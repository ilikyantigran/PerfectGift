// Verifier validates Identity-issued JWT access tokens LOCALLY against a JWKS public
// key set — no per-request call to Identity on the hot path (architecture.md §7).
//
// Identity signs with EdDSA (Ed25519); RS256 is also supported so the verifier keeps
// working if Identity's signing scheme changes. The key set is fetched from Identity's
// JWKS URL and cached; a fetch failure serves the last-known good keys (fail-soft),
// while an unverifiable or expired token is always rejected (fail-closed). Verification
// is in-house (crypto/ed25519 + crypto/rsa) so there is no external JWT dependency and
// it is fully unit-testable offline.
//
// (Ported from the API gateway's verifier so Poll authenticates the very same tokens.)
package auth
...
// Claims is the validated subset of JWT claims Poll cares about.
type Claims struct {
	Subject string
	Issuer  string
	JTI     string
...
// Source loads a JWKS. NewHTTPSource fetches from Identity's JWKS URL; StaticKeySet is
// used in tests.
...
type Config struct {
	...
	Audience   string
	Source     Source
	RefreshTTL time.Duration // default 10m
}
...
// NewVerifier builds a Verifier. The initial JWKS is fetched lazily on first Verify.
...
// claims. Returns validated Claims or an error; any error means "reject".
